    ...
    fn handle_key(&mut self, key: KeyCode) -> Option<Message> {
        ...
        if message.is_none() {
            message = match key {
                ...
                KeyCode::Char('v') | KeyCode::Char('3') => Some(Message::AppMessage(
                    AppMessage::SelectTab(Tab::Vcs),
                )),
    ...
    fn request_sort(&mut self, sort_by: SortBy) -> Option<Message> {
        ...
        match self.layers.last_mut() {
            Some(UiLayer::Tab) => match self.selected_tab {
                ...
                Tab::Vcs => Some(Message::VcsTab(
                    <VcsTab as Component>::Message::ApplySort(sort_by),
                )),
            },
            _ => None,
        }
