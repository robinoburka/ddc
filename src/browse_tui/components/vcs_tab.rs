    ...
    fn render(&mut self, frame: &mut Frame, area: Rect) {
        ...
        .block(
            Block::default()
                ...
                .title_bottom(
                    Line::from(" Use 'x' to navigate to discovered projects ")
...
fn create_row<'a>(result: &'a EnrichedVcsResult) -> Row<'a> {
    let detected_projects_size = result.matched_projects.iter().map(|p| p.size).sum();
    ...
    Row::new(vec![
        ...
        size_cell(
            result
                ...
                .saturating_sub(detected_projects_size),
        ),
        dimmed_size_cell(result.vcs_size),
        if result.matched_projects.is_empty() {
            Cell::from("")
        } else {
            size_cell(detected_projects_size)
        },
        Cell::from(detected_projects_summary(result)),
    ])
}

fn detected_projects_summary(result: &EnrichedVcsResult) -> String {
    let detected_count = result.matched_projects.len();
    if detected_count == 0 {
        return String::new();
    }
    ...
    for project in &result.matched_projects {
        let icon = format!("{}", project.lang);
        if seen.insert(icon.clone()) {
            icons.push(icon);
        }
    }

    format!("{detected_count}: {}", icons.join(" "))
}
